...
export class DataService{
    ...
    getDriverDetails(driverId){
           return this._jsonp.get('http://ergast.com/api/f1/2013/drivers/'+ driverId +'/driverStandings.json?callback=JSONP_CALLBACK',{headers:this._headers})
        .map((res)=>res.json());     
    }
    
    getDrivers() {
        return this._jsonp.get('http://ergast.com/api/f1/2013/driverStandings.json?callback=JSONP_CALLBACK',{headers:this._headers})
        .map((res)=>res.json());        
    ...
    findById(employeeId) {
        return this.employees[employeeId - 1];       
    }
    ...
    findByManager(managerId) {
         return this.employees.filter((emp) =>{            
            return parseInt(managerId) === emp.managerId;
        });       
    }
}
